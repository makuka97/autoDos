...
class App:
    ...
    def _build_buttons(self):
        ...
        ttk.Button(bar, text="＋  Add Zip",
                   style="Pill.TButton",
                   command=self._add_archive).pack(side=tk.LEFT)
    ...
    def _launch_entry(self, entry: dict):
        """Launch a game entry via DOSBox in a background thread."""
        if not self.dosbox:
        ...
        cycles  = entry.get("cycles", "auto")
        memsize = snap_memsize(int(entry.get("memsize", 16)))
        xms     = "true" if entry.get("xms", True) else "false"
        ems     = "true" if entry.get("ems", True) else "false"

        # Normalize ExoDOS cycles: "max limit 35000" or "max 35000" -> "35000"
        import re as _re
        cycles_str = str(cycles)
        ...
        if limit_match:
            ...
            # DOSBox Staging doesn't accept "auto" via -c, use a high fixed value
            conf_cycles = "max"
        else:
            conf_cycles = cycles_str

        # Build autoexec commands — on Windows no shell escaping needed,
        # subprocess passes each -c argument directly to DOSBox
        # Each -c value is passed as a single argument to DOSBox.
        # DOSBox Staging on Windows parses these directly — no shell escaping.
        # Wrap paths with spaces in double quotes inside the string itself.
        exe_dir_str = str(exe_dir)
        mount_cmd   = f'mount c "{exe_dir_str}"' if " " in exe_dir_str else f"mount c {exe_dir_str}"
    ...
    def _remove_selected(self):
        """Remove selected game from library with confirm dialog."""
        sel = self.listbox.curselection()
        if not sel:
            return
        entry = self.library[sel[0]]
